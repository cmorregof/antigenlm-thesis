...
Como visualizaciones no lineales exploratorias, calculamos proyecciones t-SNE tanto sobre la caché estratificada existente de 20,000 embeddings (10,000 H1N1 y 10,000 H3N2; semilla 42) como sobre la caché completa de 111,756 registros. Los embeddings se estandarizaron, se redujeron a 50 componentes principales para denoising y eficiencia computacional, y se proyectaron con t-SNE Barnes-Hut usando perplexity 50, inicialización PCA, 1,000 iteraciones y semilla aleatoria 42. Estas visualizaciones son solo descriptivas y no se usan para estimar dimensión intrínseca, distancia filogenética, similitud antigénica ni desempeño de pronóstico. Dado que t-SNE reoptimiza todo el mapa bidimensional para cada conjunto de entrada, las diferencias en la disposición global entre las proyecciones estratificada y completa no se interpretan como topología biológica.
...
Como comparación visual con la proyección lineal de PCA, la Figura~\ref{fig:tsne-2d} muestra una proyección t-SNE exploratoria de la caché estratificada de 20,000 registros. La proyección separa muchos vecindarios H1N1 y H3N2 y muestra clústeres locales estructurados temporalmente, en consistencia con los análisis moleculares y temporales locales anteriores. Debido a que t-SNE distorsiona fuertemente la geometría global y depende del muestreo y los hiperparámetros, esta figura se usa solo como visualización cualitativa, no como evidencia de preservación cuantitativa de distancias.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/tsne_20k_by_subtype_year.pdf}
\caption{Proyección t-SNE exploratoria de 20,000 embeddings cacheados de AntigenLM, muestreados como 10,000 registros por subtipo desde la caché estratificada existente. Los embeddings se estandarizaron, se redujeron a 50 componentes principales y se proyectaron con t-SNE Barnes-Hut usando perplexity 50 y semilla 42. La figura es solo descriptiva: los vecindarios t-SNE no deben interpretarse como distancias filogenéticas cuantitativas, relaciones antigénicas ni evidencia de pronóstico.}
\label{fig:tsne-2d}
\end{figure}

La Figura~\ref{fig:tsne-full} repite el mismo procedimiento de visualización sobre la caché completa de 111,756 registros. La proyección completa muestra islas locales más fragmentadas y una disposición global distinta a la proyección estratificada, como se espera cuando t-SNE se reoptimiza sobre un conjunto más grande y desbalanceado por subtipo. El mensaje cualitativo no cambia: los vecindarios locales muestran estructura fuerte por subtipo y parches temporales, pero el mapa no constituye evidencia de topología global de variedad ni de estructura evolutiva proporcional a distancias.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figures/tsne_full_111756_seed42_by_subtype_year.pdf}
\caption{Proyección t-SNE exploratoria sobre la caché completa de 111,756 embeddings cacheados de AntigenLM. Se usaron el mismo preprocesamiento y los mismos parámetros t-SNE que en la Figura~\ref{fig:tsne-2d}: estandarización de coordenadas, reducción a 50 componentes principales, t-SNE Barnes-Hut con perplexity 50, inicialización PCA, 1,000 iteraciones y semilla 42. El cambio en la disposición global respecto a la proyección de 20,000 registros refleja la dependencia de t-SNE frente a densidad de entrada y optimización, y no debe interpretarse como topología biológica, distancia filogenética cuantitativa, estructura antigénica ni evidencia de pronóstico.}
\label{fig:tsne-full}
\end{figure}
