...
The local analysis pipeline is script-based. Embeddings were produced by \path{latent_geometry.py}, full-data metrics and figures by \path{latent_geometry_full_analysis.py}, and the paper package by \path{make_paper_1_latent_geometry_full.py}. The focused robustness panel was run with \path{paper_revision_outputs/run_robustness_panel.py}. The primary aggregate results are stored in \path{results/latent_geometry_full_metrics.json} and summarized in \path{results/latent_geometry_full_summary.md}; robustness results are stored in \path{paper_revision_outputs/robustness_panel_results.json} and \path{paper_revision_outputs/robustness_panel_summary.md}. The manuscript tables and figures are generated from aggregate outputs and copied into the paper directory.
